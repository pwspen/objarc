...
def fft_cross_correlation(
    image_a: np.ndarray, image_b: np.ndarray, enforce_color_range: bool = True
) -> np.ndarray:
...
def cross_correlation(
    image_a: np.ndarray, image_b: np.ndarray, center: bool = True
) -> np.ndarray:
...
def best_matches(
    grid: np.ndarray, template: np.ndarray
) -> list[tuple[float, int, int]]:  # match score, x, y
    template_pixels = np.sum(template != EMPTY_COLOR)
    matches = fft_cross_correlation(grid, template)
    best = np.max(matches)
...
def extract_local_maximums(
    heatmap: np.ndarray, size: int = 3, threshold: float = 0
) -> np.ndarray:
...
def find_regions(img: np.ndarray, connectivity: int = 8) -> list[np.ndarray]:
    ...
    if img.ndim == 3:
        binary = (
            img.any(axis=2) if img.dtype == bool else img.mean(axis=2) > 0
        ).astype(np.uint8)
    else:
...
def entropy_filter(image: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    if out_h <= 0 or out_w <= 0:
        ...
        print_matrix(mask)
        raise ValueError(
            "Mask must not be larger than image in any dimension (valid mode)."
        )
